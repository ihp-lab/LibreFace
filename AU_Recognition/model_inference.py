...
class model_inference(nn.Module):
	...
	def get_inference_only_data_loaders(self):
		data_root = self.config.data_root	# /AU_Recognition/data
		data_csv = os.path.join(data_root, 'data.csv')		# has Image's Addresses
		print(data_csv)
		self.data_run_loader = get_inference_only_data_loader(data_csv, self.config)	
	...
	def run_model(self, data_run_loader):
		with torch.no_grad():		# .no_grad() - when Inference - disable gradient calculation to save memory
			self.eval()
			pred_list = []		# Prediction List
			
			start_time = time.time()
			for (images) in tqdm(data_run_loader):		##
				images = images.cuda()
				batch_size = images.shape[0]
				if self.config.half_precision:		# Float16
					images = images.half()
				...
				labels_pred = self.model(images)		# **Label Prediction by Model: [AU_1, AU_2, ..., AU_12]
	...
	def run(self):
		...
		pred_list_df.to_csv('AU_Recognition\data\pred_list.csv')
